...
int main() {
	...
	cout << "OMP fast bilateralfilter total time = " << (end - start) / 1000.0 << "seconds" << endl;
...
#pragma omp parallel for reduction(+:maxk)
...
#pragma omp parallel for
	for (int i = 0; i < size.height; i++)
	{
		for (int j = 0; j < size.width; j++)
		{
			...
			float sum = 0, wsum = 0;
			int val0 = sptr[j];
#pragma omp parallel for reduction(+:sum, wsum)
